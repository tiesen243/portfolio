...
export function mdxComponents() {
  return {
    ...
    code: ({ ...props }: Props) => (
      <code
        className='border-accent font-mono [&:not(:has(span))]:relative [&:not(:has(span))]:w-fit [&:not(:has(span))]:bg-accent/20 [&:not(:has(span))]:px-[0.3rem] [&:not(:has(span))]:py-[0.2rem] [&:not(:has(span))]:text-sm [&:not(:has(span))]:font-medium [&:not(:has(span))]:text-accent-foreground'
        {...props}
      />
    ),
