...
<Callout type="info">
You can run the following command to automatically setup all config files. Or you can follow the steps below to install each package manually.
...
```bash
cp -r ~/dotfiles/themes/.fonts ~/.local/share
```
