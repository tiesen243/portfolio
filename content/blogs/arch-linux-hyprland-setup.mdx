...
```bash
yay -S zsh hyprpaper hyprpicker hypridle hyprlock pavucontrol brightnessctl playerctl vlc floorp-bin cliphist wl-clipboard grim slurp lsd bat nwg-look fastfetch btop github-cli docker ripgrep noto-fonts noto-fonts-cjk noto-fonts-emoji
```
...
5. Optional: Install `miniconda3` for python and `nvm` for nodejs development environment and `obs-studio` for screen recording
...
- If time is not correct on windows when you dual boot with Windows 11, you can run the following command to fix the time

```bash
timedatectl set-local-rtc 1
```

- If you want to change the default shell to `zsh`, you can run the following command
