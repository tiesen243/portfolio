...
export class Password {
  public hash(password: string): string {
    ...
    return encodeBase32LowerCase(sha3_256(new TextEncoder().encode(saltedPassword)))
  }
    ...
    switch (req.method) {
      ...
      case 'GET':
        if (url.pathname === '/api/auth') {
          ...
          if (!isCallback) {
            const provider = this.providers[String(url.pathname.split('/').pop())]
            ...
            const authorizationUrl = provider.createAuthorizationURL(state, codeVerifier)

            response = NextResponse.redirect(new URL(authorizationUrl, req.nextUrl))
            response.cookies.set('code_verifier', codeVerifier)
            response.cookies.set('oauth_state', state)
          } else {
            const provider = this.providers[String(url.pathname.split('/').slice(-2)[0])]
            if (!provider) {
              response = NextResponse.json(
                { error: 'Provider not supported' },
                { status: 404 },
